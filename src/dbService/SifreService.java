...
public class SifreService {
    ...
    public dbOld.Sifre getSifre(long id) {
        try {
            EntityManager em = emf.createEntityManager();
            Query q = em.createQuery("SELECT s FROM Sifre s WHERE s.id = :id");
            q.setParameter("id", id);

            dbOld.Sifre res = (dbOld.Sifre) q.getSingleResult();
            em.close();

            return res;
        } catch (Exception ex) {
            dbOld.Sifre markabetona = new dbOld.Sifre();
    ...
    public void setSifra(long id, String sifraFrakcija04, String sifraFrakcija48, 
        ...
        try {
            EntityManager em = emf.createEntityManager();
            dbOld.Sifre p = em.find(dbOld.Sifre.class, id);
